...
# --- DOWNLOADER BLINDÉ (User-Agent iPhone) ---
def download_video(url):
    ydl_opts = {
        ...
        # ON SE DEGUISE EN IPHONE POUR TROMPER INSTAGRAM
...
def generate_recipe_from_text(text_description):
    """Fonction de secours si la vidéo bloque"""
    try:
        model = genai.GenerativeModel("gemini-2.5-flash")
        prompt = f"""
        Crée une recette basée sur ce texte : "{text_description}".
        INSTRUCTION: Recette complète, saine, note sévère.
    ...
    if st.button("Analyser"):
        if url:
            with st.status("Analyse...", expanded=True) as status:
                ...
                # CAS 1 : LA VIDÉO EST TÉLÉCHARGÉE
                if video_path:
                ...
                # CAS 2 : ECHEC (Instagram bloque) -> ON PASSE EN MANUEL
                else:
                    status.update(label="Instagram bloque (Sécurité)", state="error")
                    st.warning("⚠️ Instagram a bloqué le robot (Sécurité 'Anti-Bot').")
                    st.info("💡 Pas de panique ! Copie le texte de la description ou le nom du plat ci-dessous :")
                    
                    # On stocke l'état pour afficher le champ de secours
                    st.session_state.show_manual_input = True

    # ZONE DE SECOURS (PLAN B)
    if st.session_state.get('show_manual_input'):
        st.divider()
        manual_text = st.text_area("📋 Colle la description ici (ou dis 'Pâtes Carbonara') :")
        if st.button("Lancer avec le texte"):
            with st.spinner("Génération depuis le texte..."):
                ...
                if "error" in recipe: st.error("Erreur")
                    ...
                    st.session_state.show_manual_input = False # Reset
                    st.rerun()
...
with tabs[4]:
    ...
    # CALCULATEURS & WIKI (COPIES DU CODE V11 - VERSION COMPLETE)
    c1, c2 = st.columns(2)
    with c1:
    ...
    # WIKI COMPLET (RESTITUÉ)
    with st.expander("🥩 LES PROTÉINES (Le Constructeur)"):
        st.markdown("""
        **Rôle :** Construire le muscle, réparer les tissus, couper la faim.
        **Combien ?** 1.6g à 2g par kg.
        **Sources :** Poulet, Boeuf 5%, Poisson, Oeufs, Skyr, Lentilles.
        """)
    with st.expander("🍞 LES GLUCIDES (Le Carburant)"):
        st.markdown("""
        **Rôle :** Énergie pour l'entraînement.
        **✅ Les Bons :** Avoine, Riz Basmati, Patate Douce, Pâtes Complètes.
        **❌ A bannir :** Sucre blanc, Sodas.
        """)
    with st.expander("🥑 LES LIPIDES (Le Protecteur)"):
        st.markdown("""
        **Rôle :** Hormones, cerveau.
        **✅ Bons Gras :** Huile d'Olive (cru), Avocat, Noix, Saumon.
        """)
    st.subheader("🛑 DO & DON'T")
    c_do, c_dont = st.columns(2)
    with c_do: st.success("**✅ DO**: Légumes (Volume), Sommeil 8h, Marcher 10k pas.")
    with c_dont: st.error("**❌ DON'T**: Boire ses calories, Régimes famine.")
